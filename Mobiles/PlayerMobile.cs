...
namespace Server.Mobiles
{
	...
	public class PlayerMobile : Mobile, IHonorTarget
	{
		...
		*/
		...
		{
			get{ return GetFlag( PlayerFlag.NormalMode ); }
			set{ SetFlag( PlayerFlag.NormalMode, value ); InvalidateMyRunUO(); }
		}
		...
		public override int MaxWeight { get { return (((Core.ML && this.Race == Race.Human) ? 100 : 40) + (int)(4 * this.Str)); } }
		...
		{					
			if ( DateTime.Now >= m_NextSound && Utility.RandomBool() && MyServerSettings.EnableAmbientSoundEffects() && !(Worlds.IsOnBoat( this )) )
			{		
				Region reg = Region.Find( this.Location, this.Map );
				string category = "";
				int sound =0;
				
				if ( reg.IsPartOf( typeof( TownRegion ) ) ){										category = "city"; }
				...
				m_NextSound = (DateTime.Now + TimeSpan.FromSeconds( Utility.RandomMinMax(30, 240)) );	
			} 
		...
		private class SbResTick : Timer
		{
			...
			public SbResTick( PlayerMobile from ) : base( TimeSpan.FromMinutes( 3 ) )
			{
				m_From = from;
		...
		private void ToggleItemInsurance_Callback( Mobile from, object obj )
		{
			...
			else if ( item.LootType == LootType.Ensouled || item.LootType == LootType.Blessed || item.LootType == LootType.Newbied || item.BlessedFor == from )
			{
				BeginTarget( -1, false, TargetFlags.None, new TargetCallback( ToggleItemInsurance_Callback ) );
		...
		public override bool CheckTrade( Mobile to, Item item, SecureTradeContainer cont, bool message, bool checkItems, int plusItems, int plusWeight )
		{
			...
			/*
			if (!this.HardCore && item.IsNormalOnly && !item.IsCrafted && to is PlayerMobile && ((PlayerMobile)to).HardCore) 
				msgNum = 1061234; //You are unable to trade this normal only item with a soulbound player
*/
			if ( msgNum != 0 )
			{
				if ( message )
		...
		protected override void OnLocationChange( Point3D oldLocation ) // +++
		{
			...
			if ( !speedAble )
			{
				if ( !Server.Mobiles.AnimalTrainer.AllowMagicSpeed( this, Region.Find( this.Location, this.Map ) ) )
				{
					...
					if (this.HardCore && m_sbmaster && m_sbmasterspeed)
					{
						this.Send(SpeedControl.Disable);
						this.SendMessage( "Your speed is diminished here." );
						m_sbmasterspeed = false;
					}
					else if ( (shoes is BootsofHermes && shoes.Weight < 5.0 && !this.HardCore) )
					{
						this.Send(SpeedControl.Disable);
						if (shoes != null)
						{
			...
			else if ( speedAble && !mountAble && !Mounted && Alive )
			{
				...
				else if ( (shoes is BootsofHermes && shoes.Weight > 3.0 && !this.HardCore) )
				{
					if (shoes != null)
					    shoes.Weight = 3.0;
			...
			else if ( mountAble && !Mounted && Alive )
			{
				...
				if ( !(Server.Spells.Ninjitsu.AnimalForm.UnderTransformation( this as Mobile )) )
		...
		public override void OnDamage( int amount, Mobile from, bool willKill )
		{
			...
			if ( from != null && ((double)this.Hits / (double)this.HitsMax) <= 0.10 && this.BodyMod == 263 ) // morphing armors
				MinotaurCloak.UseMinotaurAbility( this );

			if ( from != null && ((double)this.Hits / (double)this.HitsMax) <= 0.80 && this.BodyMod == 318 )
			{
				if (Utility.RandomDouble() <= 0.25)
				{
					...
					if (from.Hits <= 5000 || Utility.RandomDouble() < 0.1)
						from.Kill();
					if (from.Hits > 5000)
						from.Hits -= 5000;
				}
		...
		private bool FindItems_Callback(Item item)
		{
			if (!item.Deleted 
				&& (
					(item.LootType == LootType.Blessed || item.Insured == true && !this.HardCore)
						||
					(item.LootType == LootType.Ensouled && this.HardCore)
					)
				)
			{
		...
		public override bool OnBeforeDeath()
		{
            ...
            //Zombiex and canchew
            if (FindMostRecentDamager(true) is BaseCreature )
            {
				...
				if (mo is Zombiex || mo.CanInfect || mo is WanderingConcubine)
				{
			...
			if (Backpack != null && !Backpack.Deleted)
			{
				...
				{
					if ((this.HardCore && ilist[i].LootType == LootType.Ensouled) || !this.HardCore) {
		...
		public override void OnDeath( Container c )
		{
			...
			if (this.BalanceStatus != 0)
				...
				changechange = (double)this.Karma / 5000; 
		...
		public void ChewItem(BaseCreature killer)
		{
			...
			{
					Item ILost = HiddenTrap.GetMyItem( this );
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.Write( (int) 38 ); // version 29 balance status, 30 hardcore added, 32 soul force, 33 gauntlet levels, 34 autores

			if (m_sbmasterspeed == null)
				m_sbmasterspeed = false;
			...
			if (m_LastGauntletLevel ==  null) {
				m_LastGauntletLevel = 0;
			}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if ( this.m_soulbounddate != null && this.HardCore)
...
/*		public void SoulTouchedAura(int stacks) {
			...
			foreach ( Mobile mobile in this.GetMobilesInRange( 10 ) )
			{
				if (!(mobile is BaseConvo) && (mobile is BaseCreature) && (((BaseCreature)mobile).SecondsSoulTouched == 0)) {
					...
					} else if (this.HardCore && this.Party != null) {
...
*/
		...
		protected override bool OnMove( Direction d )
		{
			...
			if (this.InGauntlet && LastGauntletLevel > 0)
            {
            	//this.SoulTouchedAura(LastGauntletLevel);
            } else if( this.HardCore ) {
				Phylactery phylactery = this.FindPhylactery();
				if (phylactery != null) {
					phylactery.Movable = true;
				}
				if (this.Party != null) {
					Party party = (Party)this.Party;
					if (party.Members != null && party.IsHardCore()) {
						//this.SoulTouchedAura(party.Members.Count);
					}
				}
			}
		...
		public static bool MovementThrottle_Callback( NetState ns )
		{
			...
			if ( pm != null )
			{
				if ( pm.FindItemOnLayer( Layer.Shoes ) != null )
				{
					...
					if ( (shoes is BootsofHermes && !pm.HardCore) ){ return true; }
				}
				if ( Spells.Mystic.WindRunner.UnderEffect( pm ) )
				{
					return true;
				}
				if ( pm.HardCore && pm.sbmaster && pm.sbmasterspeed )
				{
					return true;
				}
