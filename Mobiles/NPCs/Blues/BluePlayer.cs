...
{ 
	...
	public override void OnMovement( Mobile m, Point3D oldLocation )
	{
	    if ( InRange( m, 4 ) && !InRange( oldLocation, 4 ) && InLOS( m ) && m is PlayerMobile )
	    {
			...
			else if ( !m.Hidden && DateTime.Now >= m_NextResurrect && m.Hits < (m.HitsMax/2) && m is PlayerMobile || DateTime.Now >= m_NextResurrect && m.Hits < (m.HitsMax/2) && m is BaseBlue )
			{
				OfferHeal( (PlayerMobile) m );
			}
			else if ( !m.Hidden && this.Combatant == null && DateTime.Now >= m_NextTalk && m is PlayerMobile ) // check if its time to talk
			{
