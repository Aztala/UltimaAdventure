...
{ 
	...
	public override void OnMovement( Mobile m, Point3D oldLocation )
	{
		...
		{
			if ( !m.Hidden && DateTime.Now >= m_NextResurrect && this.HealsYoungPlayers && m.Hits < (m.HitsMax/2) && m is PlayerMobile || DateTime.Now >= m_NextResurrect && m.Hits < (m.HitsMax/2) && m is BaseBlue )
		    {
			OfferHeal( (PlayerMobile) m );
		    }
		    else if ( !m.Hidden && this.Combatant == null && DateTime.Now >= m_NextTalk ) // check if its time to talk
		    {
