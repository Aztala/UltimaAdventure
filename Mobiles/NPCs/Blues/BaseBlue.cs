...
{ 
	...
	{ 
		...
		public override void OnThink()
		{
			...
			if ( this is BlueGuard || this is SpawnHelper || this is SpawnHelperMage || this is MercenaryGuard)
			{
				...
				if (this.Combatant != null && this.Combatant is PlayerMobile )
				{
					...
					else if ( DateTime.Now >= m_NextResurrect && (int)GetDistanceToSqrt( this.Combatant ) > 2 && (int)GetDistanceToSqrt( this.Combatant ) < 12 && InLOS( this.Combatant ) && !m_teleporting )
					{
						m_NextResurrect = DateTime.Now + ResurrectDelay;
						m_teleporting = true;
						this.Frozen = true;
		...
		public override void OnGotMeleeAttack( Mobile attacker )
		{
			...
			if ( !(this is BluePlayer) && attacker is BaseCreature && !m_freezing && !attacker.Frozen)
			{
