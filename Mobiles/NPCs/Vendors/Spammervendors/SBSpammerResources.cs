...
namespace Server.Mobiles
{
	public class SBSpammerRes : SBInfo
	{
		...
		public class InternalBuyInfo : List<GenericBuyInfo>
		{
			...
			{  
				Add( new GenericBuyInfo( typeof( PowderOfTemperament ), 150000, 20, 0x973, 0 ) );
				Add( new GenericBuyInfo( "Spined Leather", typeof( SpinedLeather ), 25, 10000, 0x1081, 0 ) );
				if (Utility.RandomBool())
					Add( new GenericBuyInfo( "Horned Leather", typeof( HornedLeather ), 35, 10000, 0x1081,0 ) );
				if (Utility.RandomDouble() > 0.80)
					Add( new GenericBuyInfo( "Barbed Leather", typeof( BarbedLeather ), 50, 5000, 0x1081, 0 ) );
				if (Utility.RandomBool())
				...
				Add( new GenericBuyInfo( typeof( BlankScroll ), 15, 10000, 0x0E34, 0 ) );	
