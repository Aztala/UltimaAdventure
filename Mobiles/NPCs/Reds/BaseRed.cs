...
{ 
	...
	{ 
		...
		public virtual void OfferHeal( PlayerMobile m )
		{
			...
			if ( DateTime.Now >= m_NextResurrect && m.Criminal && m.Kills >= 2 && !m.Hidden)
			{
		...
		public override void OnThink()
		{
			...
			if (this.Combatant != null && this.Combatant is PlayerMobile )
			{
				...
				else if ( DateTime.Now >= m_NextResurrect && (this is SpawnRed || this is DreadLord  ) && (int)GetDistanceToSqrt( this.Combatant ) > 2 && (int)GetDistanceToSqrt( this.Combatant ) < 12 && InLOS( this.Combatant ) && !m_teleporting )
				{
					m_NextResurrect = DateTime.Now + ResurrectDelay;
					m_teleporting = true;
					this.Frozen = true;
		...
		public override void OnGotMeleeAttack( Mobile attacker )
		{
			...
			if ( !(this is RedPlayer) && attacker is BaseCreature && !m_freezing && !attacker.Frozen)
			{
		...
		public void CallOthers( Mobile target )
		{
			bool callout = false;
		...
		{ 
			...
			public BandageTimer( BaseRed o ) : base( TimeSpan.FromSeconds( Utility.RandomMinMax(5,8) ) ) 
			{ 
			pk = o;
