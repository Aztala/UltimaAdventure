	...
	public partial class Ability
	{
		...
		public static void Aura( Point3D location, Map map, Mobile from, int min, int max, ResistanceType type, int range, Poison poison, string text, bool scales, bool allownull, bool effects, int itemid, int hue )
		{
			...
			foreach ( Mobile m in Map.AllMaps[map.MapID].GetMobilesInRange( location, range ) )
			{
				...
				{
					if (from is BaseCursed)
					{
						if (m is PlayerMobile && m.Karma <= -500) 
							return; // final, evil players don't get harmed
						...
						else if (m is BaseCreature ) 
						{
							...
							else if ( ((BaseCreature)m).Controlled && ((BaseCreature)m).ControlMaster is PlayerMobile )
							{
								Mobile tamer = ((BaseCreature)m).ControlMaster;
			...
			for ( int i = 0; i < targets.Count; i++ )
			{
				...
				switch( type )
				{
					case ResistanceType.Physical: AOS.Damage( m, (from == null) ? m : from, auradamage, 100, 0, 0, 0, 0 ); break;
					case ResistanceType.Fire: AOS.Damage( m, (from == null) ? m : from, auradamage, 0, 100, 0, 0, 0 ); break;
					case ResistanceType.Cold: AOS.Damage( m, (from == null) ? m : from, auradamage, 0, 0, 100, 0, 0 ); break;
					case ResistanceType.Poison: AOS.Damage( m, (from == null) ? m : from, auradamage, 0, 0, 0, 100, 0 ); break;
					case ResistanceType.Energy: AOS.Damage( m, (from == null) ? m : from, auradamage, 0, 0, 0, 0, 100 ); break;
				}
			}
		...
		*/
		...
		public static void EchoStrike( Mobile from, int min, int max )
		{
			...
			for ( int i = 0; i < mobiles.Count; i++ )
			{
				...
				else if (from != null && from.Map != null)
				{
		...
		*/
		...
		private class RallyTimer : Timer
		{
			...
			public RallyTimer( Mobile user, int delay ) : base( TimeSpan.FromMilliseconds( 100.0 ), TimeSpan.FromMilliseconds( 100.0 ) )
			{
				m_User = user;
				m_Count = 0;
				m_MaxCount = delay;
		...
		private class SlideTimer : Timer
		{
			...
			public SlideTimer( Mobile mob, Point3D point, int dist, bool push ) : base( TimeSpan.FromMilliseconds( 100.0 ), TimeSpan.FromMilliseconds( 100.0 ) )
			{
				m_Mob = mob;
				m_Point = point;
				m_Dist = dist;
			...
			protected override void OnTick()
			{
				if ( m_Mob == null )
					Stop();
				else if ( m_Count >= m_Dist )
				{
					m_Mob.CantWalk = false;
					Stop();
				}

				Direction d = m_Mob.GetDirectionTo( m_Point );
				Point3D moveto = new Point3D( m_Mob.X, m_Mob.Y, m_Mob.Z );

				if ( m_Push )
				{
					switch( d )
					{
						case (Direction)0x0: case (Direction)0x80: d = (Direction)0x4; break; // North to South
						case (Direction)0x1: case (Direction)0x81: d = (Direction)0x5; break; // Right to Left
						case (Direction)0x2: case (Direction)0x82: d = (Direction)0x6; break; // East to West
						case (Direction)0x3: case (Direction)0x83: d = (Direction)0x7; break; // Down to Up
						case (Direction)0x4: case (Direction)0x84: d = (Direction)0x0; break; // South to North
						case (Direction)0x5: case (Direction)0x85: d = (Direction)0x1; break; // Left to Right
						case (Direction)0x6: case (Direction)0x86: d = (Direction)0x2; break; // West to East
						case (Direction)0x7: case (Direction)0x87: d = (Direction)0x3; break; // Up to Down
						default: { break; }
					}
				}
