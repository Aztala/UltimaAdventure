...
namespace Server.Mobiles
{
	public class NecroMageAI : BaseAI
	{
		...
		private const double HealChance = 0.10; // 5% chance to heal at gm necromancy, uses spirit speak healing
		private const double TeleportChance = 0.10; // 5% chance to teleport at gm magery
		...
		private Spell CheckCastHealingSpell()
		{
			...
			if ( m_Mobile.Int >= 500 )
				delay = Utility.RandomMinMax( 5, 7 );
			else
				delay = Math.Sqrt( 550 - m_Mobile.Int );
		...
		public virtual Spell GetRandomDamageSpell( int dist, bool many )
		{
		...
		public virtual Spell ChooseSpell( Mobile c )
		{
			...
			string strategy = AssessStrategy( c );
			double odds = Utility.RandomDouble();

			if (strategy == "Many.LowHits" || strategy == "Many.HighHits")
			{
				if (strategy == "Many.LowHits" && Utility.RandomDouble() < 0.75)
					spell = GetRandomDamageSpell( (int)m_Mobile.GetDistanceToSqrt( c), true );
				else if (strategy == "Many.HighHits" && Utility.RandomDouble() < 0.60)
					spell = GetRandomDamageSpell( (int)m_Mobile.GetDistanceToSqrt( c ), true );
				else if ( m_Mobile.Skills[ SkillName.Poisoning ].Value > 50.0 && odds <= 0.25 && !c.Poisoned) // higher chance to poison with skill
					spell = new PoisonSpell( m_Mobile, null );
				else if ( m_Mobile.Skills[ SkillName.Poisoning ].Value <= 50.0 && odds <= 0.10 && !c.Poisoned)
					spell = new PoisonSpell( m_Mobile, null );
				else if (odds <= 40 && m_Mobile.Skills[ SkillName.Magery ].Value > 50.0 && !c.Paralyzed)
					spell = new ParalyzeSpell( m_Mobile, null );
				else if ( odds <= 0.65 && m_Mobile.Skills[ SkillName.Necromancy ].Value > 30 && BloodOathSpell.GetBloodOath( c ) != m_Mobile )
					spell = new BloodOathSpell( m_Mobile, null );
				else if ( odds <= 0.77 && ( m_Animated == null || !m_Animated.Alive ) && m_Mobile.Skills[ SkillName.Necromancy ].Value > 40 )
					spell = new AnimateDeadSpell( m_Mobile, null );
				//else if ( odds <= 0.75 && m_Mobile.Skills[ SkillName.Necromancy ].Value > 80 && ( m_Mobile.Followers + 3 ) < m_Mobile.FollowersMax )
				//	spell = new VengefulSpiritSpell( m_Mobile, null );
				//else if (odds <= 0.74 && c.Mana <= (c.ManaMax/2)) //enemy is using mana, lets drain them
			...
			if (strategy == "Close.LowHits" || strategy == "Close.HighHits" )
			{
				if (strategy == "Close.LowHits" && Utility.RandomDouble() < 0.75)
					spell = GetRandomDamageSpell( (int)m_Mobile.GetDistanceToSqrt( c ), false );
				else if (strategy == "Close.HighHits" && Utility.RandomDouble() < 0.50)
					spell = GetRandomDamageSpell( (int)m_Mobile.GetDistanceToSqrt( c ), false );
				else if ( m_Mobile.Skills[ SkillName.Poisoning ].Value > 50.0 && odds <= 0.25 && !c.Poisoned) // higher chance to poison with skill
					spell = new PoisonSpell( m_Mobile, null );
				else if ( m_Mobile.Skills[ SkillName.Poisoning ].Value <= 50.0 && odds <= 0.10 && !c.Poisoned)
			...
			if (strategy == "Distance.LowHits" || strategy == "Distance.HighHits")
			{
				if (strategy == "Distance.LowHits" && Utility.RandomDouble() < 0.70)
					spell = GetRandomDamageSpell( (int)m_Mobile.GetDistanceToSqrt( c ), false );
				if (strategy == "Distance.HighHits" && Utility.RandomDouble() < 0.40)
					spell = GetRandomDamageSpell( (int)m_Mobile.GetDistanceToSqrt( c ), false );
				else if ( m_Mobile.Skills[ SkillName.Poisoning ].Value > 50.0 && odds <= 0.35 && !c.Poisoned) // higher chance to poison with skill
					spell = new PoisonSpell( m_Mobile, null );
				else if ( m_Mobile.Skills[ SkillName.Poisoning ].Value <= 50.0 && odds <= 0.15 && !c.Poisoned)
					spell = new PoisonSpell( m_Mobile, null );
				else if (odds <= 50 && m_Mobile.Skills[ SkillName.Magery ].Value > 50.0 && !c.Paralyzed)
					spell = new ParalyzeSpell( m_Mobile, null );
				else if ( odds <= 0.55 && m_Mobile.Skills[ SkillName.Necromancy ].Value > 30 && BloodOathSpell.GetBloodOath( c ) != m_Mobile )
					spell = new BloodOathSpell( m_Mobile, null );
				else if ( odds <= 0.60 && ( m_Animated == null || !m_Animated.Alive ) && m_Mobile.Skills[ SkillName.Necromancy ].Value > 40 )
					spell = new AnimateDeadSpell( m_Mobile, null );
				else if ( odds <= 0.75 && m_Mobile.Skills[ SkillName.Necromancy ].Value > 80 && ( m_Mobile.Followers + 3 ) < m_Mobile.FollowersMax )
					spell = new VengefulSpiritSpell( m_Mobile, null );
				else if (odds <= 0.80 && c.Mana <= (c.ManaMax/2)) //enemy is using mana, lets drain them
					spell = GetRandomManaDrainSpell();
				else if (odds <= 0.90)
					spell = GetRandomCurseSpell();
				else 
...
/*
			switch ( Utility.Random( 25 ) )
			{
				...
				default: // Damage them.
				{
					...
					spell = GetRandomDamageSpell( (int)m_Mobile.GetDistanceToSqrt( c ) );
					break;
				}
			}*/
		...
		private TimeSpan GetDelay()
		{
			...
			double newtime = 1+ ( (m_Mobile.Hits/m_Mobile.HitsMax) * ( min + ((max - min) * Utility.RandomDouble()) ) );

			return TimeSpan.FromSeconds( newtime );
		}
