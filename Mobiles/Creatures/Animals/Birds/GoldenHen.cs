...
namespace Server.Mobiles
{
	...
	public class GoldenHen : BaseCreature
	{
		...
		{
			if ( DateTime.Now > ( m_pluckedtime + TimeSpan.FromHours( 23.0 )) && ((!m_plucked && Utility.RandomDouble() <= 0.05) || (m_plucked && Utility.RandomDouble() < 0.02)) )
			{
			...
			if ( DateTime.Now > ( m_pluckedtime + TimeSpan.FromHours( 23.0 )) && this.Controlled && this.ControlMaster != null && from == this.ControlMaster ) 			
			{
