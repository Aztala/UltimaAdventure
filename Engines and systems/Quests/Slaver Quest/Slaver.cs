...
namespace Server.Mobiles
{
	...
	public class Slaver : Mobile
	{
		...
		public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			...
			if ( InRange( m, 4 ) && !InRange( oldLocation, 4 ) && InLOS( m ) && m is PlayerMobile && !m.Hidden && DateTime.Now >= m_NextTalk)
			{
			...
			if (InRange( m, 8 ) && !InRange( oldLocation, 8 ) && InLOS( m ) && m is BaseChild )
			{
