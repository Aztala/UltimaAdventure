			...
			if ( Utility.RandomBool() ) // STEAL FROM TOWN
			{
				...
				for ( int i = 0; i < targets.Count; ++i )
				{
					...
					if ( xCount == aCount )
					{
						...
						note.NoteReward = (int)((double)( ( rewardMod * (int)(m.Skills[SkillName.Stealing].Value * 10) ) + ( Utility.RandomMinMax( 0, 30 ) * (int)(m.Skills[SkillName.Stealing].Value * 2) ) ) * (1+ ((double)note.Consecutive/125)));
			...
			else // STEAL FROM DUNGEON
			{
				...
				for ( int i = 0; i < targets.Count; ++i )
				{
					...
					if ( xCount == aCount )
					{
						...
						note.Consecutive = consecutive;
						note.NoteReward = (int)((double)(( difficult * (int)(m.Skills[SkillName.Stealing].Value * 10) ) + ( Utility.RandomMinMax( 0, 30 ) * (int)(m.Skills[SkillName.Stealing].Value * 2) )) * (1+ ((double)note.Consecutive/20)));
						note.NoteReward = (int)( ((double)Server.Misc.MyServerSettings.QuestRewardModifier(note.NoteOwner, null) * 0.01) * note.NoteReward );
					}
				}
			}
