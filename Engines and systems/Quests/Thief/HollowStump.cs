			...
			if ( from.InRange( this.GetWorldLocation(), 2 ) )
			{
				...
				if ( from.Backpack.FindItemByType( typeof ( ThiefNote ) ) != null )
				{
					...
					if ( envelope.NoteOwner == from && envelope.NoteItemGot > 0 && StumpTown == envelope.NoteDeliverTo && envelope.NoteDeliverType == 1 )
					{
						...
						if (envelope.NoteReward < 7500)
							from.AddToBackpack ( new Gold( envelope.NoteReward ) );
						else 
							from.AddToBackpack ( new BankCheck( envelope.NoteReward ) );
						
						Titles.AwardFame( from, ((int)(envelope.NoteReward/10)), true );
						Titles.AwardKarma( from, -((int)(envelope.NoteReward/10)), true );
						...
						else if (Utility.RandomDouble() < (0.05 + ((double)envelope.Consecutive / 150)) )
						{
							switch ( Utility.Random( 6 ) ) //
										{					
												case 0: rngitem = Loot.RandomArty(); break;
												case 1: rngitem = Loot.RandomArmorOrShieldOrWeaponOrJewelryOrClothing(); Stealing.ItemMutate( from, from.Luck, rngitem, (int)(envelope.Consecutive/2)); break;
												case 2: rngitem = Loot.RandomInstrument(); Stealing.ItemMutate( from, from.Luck, rngitem, (int)(envelope.Consecutive/2) ); break;
												case 3: rngitem = Loot.RandomQuiver(); Stealing.ItemMutate( from, from.Luck, rngitem, (int)(envelope.Consecutive/2) ); break;
												case 4: rngitem = Loot.RandomWand(); Stealing.ItemMutate( from, from.Luck, rngitem, (int)(envelope.Consecutive/2) ); break;
												case 5: rngitem = Loot.RandomJewelry(); Stealing.ItemMutate( from, from.Luck, rngitem, (int)(envelope.Consecutive/2) ); break;
										}
							from.AddToBackpack( rngitem );
							from.SendMessage( "As a reward for your dedication, the guild offers you a special gift.  May it help you stay in the shadows." );
						}
					}
				}
