		...
		public void EndCombine( Mobile from, object o )
		{
			if ( o is Item && ((Item)o).IsChildOf( from.Backpack ) )
			{
				if ( o is SmallBOD )
				{
					...
					int matmult = 2;
					if (m_Material <= BulkMaterialType.ShadowIron )
						matmult = 10;
					else if (m_Material <= BulkMaterialType.Bronze )
						matmult = 150;
					else if (m_Material <= BulkMaterialType.Agapite)
						matmult = 750;
					else if (m_Material <= BulkMaterialType.Verite)
						matmult = 2500;
					else if (m_Material <= BulkMaterialType.Valorite )
						matmult = 7000;
					else if (m_Material <= BulkMaterialType.Spined)
						matmult = 10;
					else if (m_Material <= BulkMaterialType.Horned )
						matmult = 1000;
					else if (m_Material <= BulkMaterialType.Barbed )
						matmult = 7000;

					int cost = matmult * m_AmountMax;
					if (m_RequireExceptional )
						cost *= 3;
