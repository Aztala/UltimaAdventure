...
namespace Server
{
	...
	public class GauntletMaster : Banker
	{
		...
		public static string RegionName { get { return "The Gauntlet Arena"; } }
...
if (m_Region != null)
					...
					if (this.Map.Regions.ContainsKey(GauntletMaster.RegionName))
						this.Map.UnregisterRegion(m_Region);

m_Region = new MBRegion(this, GauntletMaster.RegionName, EventMap, 200, m_SpawnArea);
					m_Region.Register();
...
*/
		...
		public void StartEvent(Mobile m)
		{
			...
			if (m is PlayerMobile) {
				...
				gauntletLevel = ((PlayerMobile)m).LastGauntletLevel;
			} 
			m_IsActive = true;
			...
			m_MBEntry = new MonstersBashEntry() { Player = m, Score = 0, BashType = MonstersType, Level = gauntletLevel };
		...
		public void OnEventFinished(bool isWin)
		{
			...
			{				
				if (isWin)
				{
					...
					rewardbox.DropItem( new Gold( (int)( 1000 * ( Math.Pow(1.25, (int)((PlayerMobile)pm).LastGauntletLevel ) ) ) ) );
