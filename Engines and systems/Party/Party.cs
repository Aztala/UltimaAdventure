...
namespace Server.Engines.PartySystem
{
	public class Party : IParty
	{
		...
		public Party( Mobile leader )
		{
            ...
            if (leader is PlayerMobile && ((PlayerMobile)leader).HardCore) {
		...
		public void UpdateSoulboundBuffs() {
			...
			for ( int i = 0; i < m_Members.Count; ++i )
			{
				...
				if (f is PlayerMobile && ((PlayerMobile)f).HardCore) {
						...
						if (IsHardCore()) {
							...
							if (mods.Count > 0) {
								foreach(PhylacteryMod mod in mods) {
									mod.Offset = (baseOffset*m_Members.Count);
								}
		...
		public void Remove( Mobile m )
		{
			...
			// if they are hardcore make sure their buffs are removed if they had them
			if (m is PlayerMobile && ((PlayerMobile)m).HardCore) {
		...
		public void Disband()
		{
			...
			if (m_Leader is PlayerMobile && ((PlayerMobile)m_Leader).HardCore) {
				this.ClearSoulboundPartyBuffs(((PlayerMobile)m_Leader));
			}
			m_Members.Clear();
		}
		...
		public static void Invite( Mobile from, Mobile target )
		{
			...
			if (from is PlayerMobile && target is PlayerMobile) {
				if ( 
					(((PlayerMobile)from).HardCore && !((PlayerMobile)target).HardCore)
					|| (((PlayerMobile)target).HardCore && !((PlayerMobile)from).HardCore)
					) {
