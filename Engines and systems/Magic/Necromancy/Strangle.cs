	...
	public class StrangleSpell : NecromancerSpell
	{
		...
		private class InternalTimer : Timer
		{
			...
			public InternalTimer( Mobile target, Mobile from ) : base( TimeSpan.FromSeconds( 0.1 ), TimeSpan.FromSeconds( 0.1 ) )
			{
				...
				int nBenefit = 0;
				if ( from is PlayerMobile ) // WIZARD
				{
					nBenefit = (int)(from.Skills[SkillName.Necromancy].Value / 5);
				}

				double spiritLevel = (from.Skills[SkillName.SpiritSpeak].Value / 20) + nBenefit;
