...
/* Tele To */			{ true,			true,			true,			true,		true,			true,			true,		true,				true,				true,				true, 			true,			true,				true,				true,			true },
		...
		public static bool CheckTravel( Mobile caster, Map map, Point3D loc, TravelCheckType type )
		{
			...
			// Final: Can't teleport into a dungeon (Gadget: is special case for Doom Gauntlet truly necessary?)
		...
		public static void Damage( Spell spell, TimeSpan delay, Mobile target, Mobile from, double damage, int phys, int fire, int cold, int pois, int nrgy, DFAlgorithm dfa )
		{
			...
			if (from is PlayerMobile && ((PlayerMobile)from).NormalMode)
				iDamage = AdventuresFunctions.DiminishingReturns( iDamage , 225, 10 ); // FINAL diminishing returns
			else if (from is PlayerMobile )
				iDamage = AdventuresFunctions.DiminishingReturns( iDamage , 175, 10 ); // FINAL diminishing returns
			else 
				iDamage = AdventuresFunctions.DiminishingReturns( iDamage , 200, 10 ); // FINAL diminishing returns
