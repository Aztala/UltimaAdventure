   ...
   { 
	...
	private void SellPetForGold(Mobile from, BaseCreature pet, int goldamount)
	{
		...
		double chance = pet.MinTameSkill/150;
					...
					if (odds <= ( 0.01 * chance) )
					{
						...
						backpack.DropItem( new PetSlotDeed() );
						from.Say( "I have an extremely special gift for you today... " );
					}		
					else if (odds <= (0.05 * chance) )
					{
						if (Utility.RandomBool())
						{
						backpack.DropItem( new PetBondDeed() );
						from.Say( "Here... I have something special for you today... " );
						}
					}

					else if (odds <= (0.12 * chance) )
					{
						backpack.DropItem( new PetControlDeed() );
						from.Say( "Here... have this as an extra... " );
					}
    ...
    { 
		...
		{ 
			...
			petprice *=  ((double)Misc.MyServerSettings.GetGoldCutRate(this, null)/100); // tie it to the doomcurse

			if (pet.Level > 1) // increased price for each level
			{
				petprice /= ( ( Math.Pow( pet.Level, 0.33 ) ) / ((double)pet.Level) ); // change exponent > 0 && < 1. higher means less gold. 0.15 was too small, increased to 0.33
			}
