...
namespace Server.Items
{
	...
	public class Phylactery : Item
	{
		...
		public int GetRawPoints(Mobile from) {
			...
			rawPoints += MyServerSettings.EnhancePotionCap();
		...
		public int CalculateExtraGold(int baseGoldAmount) {
			int goldFind = 0;
			if (ImpEssence > 0) {
				goldFind = ImpEssence + Utility.RandomMinMax(5,50);
			}
		...
		public void SetAttributeCaps() {
			...
			ImpEssenceMax = PhylacteryPowerModifier(MyServerSettings.EnhancePotionCap());
		...
		public void MergePhylactery(PlayerMobile player, Phylactery target) {
			if ( target.Serial != this.Serial) {
				...
				if (combinedPower > PowerLevelMax ) {
					player.SendLocalizedMessage( 1061185 ); // The power of this item is too great and would destroy your phylactery
				} else {
		...
		public void DeleteEnsouledItems(PlayerMobile player) {
			List<Item> items = player.Backpack.FindItemsByType<Item>(player.GetEnsouledItems);
		...
		public void UpdateOwnerSoul(PlayerMobile player) {
			if ( Core.AOS ) {
				...
				if (enhancePotions > MyServerSettings.EnhancePotionCap()) {
					enhancePotions = MyServerSettings.EnhancePotionCap();
				}
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if ((ImpEssence  + CalculatePhylacteryMods("ImpEssence")) > 0) {
				list.Add( 1060658, "{0}\t{1}", "Gold find", (ImpEssence + 5) + "-" + (ImpEssence + 50) + "%" );  // ~1_val~: ~2_val~			
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.Write( (int) 1 ); // version 1, added temporary phylactery
