...
namespace Server.Mobiles
{
	...
	public class PlayerVendor : Mobile
	{
		...
		private static bool EnableSimulatedSales = MyServerSettings.EnableSimulatedPVSales(); // master toggle for simulated sales
		private static bool EnableRandomAds = MyServerSettings.EnableRandomPVAdvertisements(); // if true, items for sale will be advertised on AdTimer tick
		private static bool ChargeForServicePerUODay = MyServerSettings.PVChargeForServicePerUODay(); // if true, the vendor will charge money for its service every UO day (and not every real day)
		...
		public PlayerVendor( Mobile owner, BaseHouse house )
		{
			...
			InitStats( 200, 200, 200 ); // WIZARD CHANGED FROM 75 TO 200 SO WE CAN DRESS THEM
			InitBody();
			InitOutfit();
...
/// WIZARD ////////////////////////////////////////////////////////////////////////////////////
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			...
			if (!e.Handled && EnableSimulatedSales && (WasNamed(e.Speech) || Insensitive.StartsWith(e.Speech, "vendor ")) && Insensitive.Contains(e.Speech, "report") || Insensitive.Contains(e.Speech, "recent") || Insensitive.Contains(e.Speech, "sales"))
			{
				if (IsOwner( from ))
				{
			...
			if (!e.Handled && EnableSimulatedSales && EnableSpeech && IsOwner( from ) && (WasNamed( e.Speech ) || Insensitive.StartsWith(e.Speech, "vendor ")))
			{
		...
		private class RandomAdTimer : Timer
		{
			...
			foreach ( Item item in m_Vendor.Backpack.Items )
			{
			    ...
			    bool banned = item is Gold || item is DDCopper || item is DDSilver || item is DDJewels || item is DDXormite || item is DDGemstones || item is DDGoldNuggets;
				...
				foreach( Item ins in item.Items )
				{
					...
					bool insBanned = ins is Gold || ins is DDCopper || ins is DDSilver || ins is DDJewels || ins is DDXormite || ins is DDGemstones || ins is DDGoldNuggets;
		...
		private class RandomBuyTimer : Timer
		{
			...
			if (item is Container)
			{
			    ...
			    {
				bool banned = it is Gold || it is DDCopper || it is DDSilver || it is DDJewels || it is DDXormite || it is DDGemstones || it is DDGoldNuggets;
		    ...
		    protected override void OnTick()
		    {
			if (!EnableSimulatedSales || m_Vendor.Backpack == null)
			    return;
