...
namespace Server.Items
{
	public class BaseHarvester : Container, IOneTime
	{
		...
		public override void AddNameProperties( ObjectPropertyList list )
		{
			...
			{
				if (m_quality <= 3)
					list.Add( "This device is barely holding together" ); 
				else if (m_quality <= 5)
					list.Add( "This device is poorly built" ); 
				else if (m_quality <= 8)
					list.Add( "This device is well built" ); 
				else if (m_quality <= 10)
					list.Add( "This device is perfectly built" ); 
			}
		...
		{
			if (from == this.owner || this.disabled)
				return true;
		...
		{
			if (m_type < 10 && dropped is DDCopper)
			{
		...
		public virtual void DoHarvest()
		{
			...
			if (Utility.RandomDouble() < ((double)this.m_quality / 10))
			{
				if (m_type < 10)
				{
					...
					{
						if (Utility.RandomDouble() < 0.50) // 1% chance of getting to 9
							chance += 1;
					}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if (this.m_owner == null) //check placement for 1. placed on ground, 2. proximity to another device and 3. proper ground tiles
			{
				...
				if ( m_type == 10 && !(((PlayerMobile)from).NormalMode) )
					{
			...
			if (from == this.m_owner) // check on harvested items
			{
				if (!this.m_disabled)
				{
					if ( from.AccessLevel > AccessLevel.Player || from.InRange( this.GetWorldLocation(), 2 )  )
					{
						...
						if (this.MaxCount > 0 ) //owner resets the device if it was attacked
							this.MaxCount = 0;
					}
					else
					{
			...
			{			
				...
				if (m_type == 10 && ((PlayerMobile)from).BalanceStatus == 0 && m_owner.Karma >=0 && from.Karma >= 0)
				{
		...
		public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			...
			double range = 0;
			if (m is PlayerMobile && ((PlayerMobile)m).NormalMode && m_type == 10 && !this.Movable && this.m_owner != null )
			{
