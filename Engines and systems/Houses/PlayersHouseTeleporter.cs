...
namespace Server.Items
{
    public class PlayersHouseTeleporter : Item, IDyable, ISecurable
	{
		...
		public override void AddNameProperties( ObjectPropertyList list )
		{
			...
			list.Add( "Only meant to be used to teleport inside a single home" ); 
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else
			{
				...
				else
				{
					...
					from.SendMessage ("You have marked the teleporter.  You can now unlock it and move it to the location you wish to teleport from. It must be locked down in that house to use it.");
				}
			}
			return;
		}
		...
		public virtual void StartTeleport( Mobile m )
		{
			...
			else if ( m_Delay == TimeSpan.Zero )
			{
				...
				if ( ((PlayerMobile)m).NormalMode && house != housedest )
				{
		...
		{            
            ...
            else if ( m_Active )
			{
				...
				//	return true;
				//}
				else if ( m.Skills[SkillName.Magery].Base < 80.0 && m.Skills[SkillName.Necromancy].Base < 80.0 && m_MapDest == Map.Trammel && thisX > 5798 && thisY > 2727 && thisX < 6105 && thisY < 3023 ) // TO STOP TRAVEL TO THE MOON
				{
