...
namespace Server.Items
{
	...
	{
		public override int DefaultGumpID{ get{ return 0x47; } }
		...
		#region LevelItem public variable
		...
		*/
		...
		public int LowerAmmoCost
		{
			...
			set{ 
				if (this is BaseLevelQuiver)
					m_LowerAmmoCost = (int)(((double)((BaseLevelQuiver)this).Level/200) * 90);
				else
		...
		public int WeightReduction
		{
			...
			set{ 
				if (this is BaseLevelQuiver)
					m_WeightReduction = (int)(((double)((BaseLevelQuiver)this).Level/200) * 100);
				else
		...
		public int DamageIncrease
		{
			...
			set{ 
				if (this is BaseLevelQuiver)
				m_DamageIncrease = (int)(((double)((BaseLevelQuiver)this).Level/200) * 50);
				else
				m_DamageIncrease = value; InvalidateProperties(); }
		}
		...
		{
			typeof( Arrow ), typeof( Bolt ), typeof( MageEye ),typeof( Krystal ),typeof( HarpoonRope )
        };
		...
		public override bool CheckHold( Mobile m, Item item, bool message, bool checkItems, int plusItems, int plusWeight )
		{
            ...
            Item MageEye = this.FindItemByType(typeof(MageEye));
            Item Krystal = this.FindItemByType(typeof(Krystal));
            Item HarpoonRope = this.FindItemByType(typeof(HarpoonRope));
            ...
            if (MageEye != null && item is MageEye)
            {
                if (MageEye.Amount + item.Amount <= m_Capacity)
                    return true;
            }
            if (Krystal != null && item is Krystal)
            {
                if (Krystal.Amount + item.Amount <= m_Capacity)
                    return true;
            }
            if (HarpoonRope != null && item is HarpoonRope)
            {
                if (HarpoonRope.Amount + item.Amount <= m_Capacity)
                    return true;
            }
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( ItemID != 0x2B02 && ItemID != 0x2B03 ){ ItemID = 0x2B02; }
			Layer = Layer.Cloak;
			GumpID = 0x47;
		}
