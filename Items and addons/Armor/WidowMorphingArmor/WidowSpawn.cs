            ...
            else
            {
                ...
                if (from.Items != null && from.Backpack != null)
                {
                    ...
                    foreach (Item item in mobitems)
                        {
                            if (item.Layer == Layer.Backpack || item.Layer == Layer.Bank || item.Insured || item.LootType == LootType.Blessed || item.LootType == LootType.Newbied || item.LootType == LootType.Ensouled) continue;
                            EquipItem(item);
                        }
        ...
        { 
	        if (m is PlayerMobile)
            {
				if (m.AccessLevel >= AccessLevel.GameMaster)
					...
					return true;
				else if ( m == this.boss && ((PlayerMobile)m).BodyMod == 84 )
					return false;
