...
{  
	public class DarkFatherCloak : BaseCloak
	{
		...
		public static void UseMinotaurAbility( Mobile from )
		{
			...
			if (from != null)
			{
					if (DateTime.Now > ( speciallastused + TimeSpan.FromHours( 24.0 )) );
					{
						speciallastused = DateTime.Now;
						...
						if ( map != null )
						{
							...
							foreach ( Mobile m in from.GetMobilesInRange( 10 ) )
							{
								...
								if ( m is BaseCreature && !((BaseCreature)m).BardPacified )
									((BaseCreature)m).Pacify( m, DateTime.Now + TimeSpan.FromSeconds( seconds ) );
							}
						}
					}
		...
		public override void OnRemoved( object parent )
		{
			if ( parent is Mobile )
			{
				...
				if (m is PlayerMobile)
				{
					...
					m.NameHue = -1;
					m.HueMod = -1;
					m.Karma = legs.originalkarma;
					legs.originalkarma = 0;
				}
