	...
	public class ClothingBlessTarget : Target // Create our targeting class (which we derive from the base target class)
	{
		...
		protected override void OnTarget( Mobile from, object target ) // Override the protected OnTarget() for our feature
		{
			...
			if ( target is BaseClothing )
			{
				...
				if (  item.LootType == LootType.Ensouled ||  item.LootType == LootType.Blessed || item.BlessedFor == from || (Mobile.InsuranceEnabled && item.Insured) ) // Check if its already newbied (blessed)
				{
			...
			else if ( ( target is MagicCloak ) || ( target is MagicRobe ) || ( target is MagicBoots ) ) // WIZARD ADDED
			{
