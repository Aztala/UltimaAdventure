...
*/
			...
			BaseCreature from = (BaseCreature)targ;
					...
					if (!from.CanAngerOnTame) // creatures that anger are harder to tame
						finalcompute /= 1.15;
						...
						while ( finalcompute > 0 )
						{
							if (finalcompute > step)
							{
								...
								if (final < 110)
									...
									finalcompute *= (diminish * ((150 - final) / 40)); // makes sure max is 122 skill
							}
							else
							{
					...
					val = (int)Math.Ceiling(final) + (15-(15*(final/150))); // 15 addition only applies to taming value under 100
