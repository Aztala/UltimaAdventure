	...
	public class BandageContext
	{
		...
		public void EndHeal()
		{
			...
			else
			{
				...
				if ( chance > Utility.RandomDouble() )
				{
					...
					min = (anatomy / 2) + (healing / 2) + 50.0; // WIZARD
		...
		public static BandageContext BeginHeal( Mobile healer, Mobile patient )
		{
			...
			else if ( healer.CanBeBeneficial( patient, true, true ) )
			{
				...
				double dexseconds = ( 300 - dex ) /60;
				if (healer is PlayerMobile && ((PlayerMobile)healer).HardCore)
					dexseconds = ( 200 - dex ) /60;

				if (dexseconds < 1 && dexseconds > 0.5) // FINAL dex over 300 is possible
					dexseconds = 1;
				...
				if ( onSelf )
				{
					if ( Core.AOS )
					{
						...
						seconds = (bandageSpeedMin*2) + dexseconds;
				}
				else
				{
					...
					{
						if (dexseconds >= 2)
							seconds = dexseconds; // FINAL why should vet skill take 2 seconds regardless of dex?
						else
							seconds = 2;
					}
					else if ( Core.AOS )
