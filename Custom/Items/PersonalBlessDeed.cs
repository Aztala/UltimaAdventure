    ...
    public class PersonalBlessTarget : Target // Create targeting class
    {
        ...
        protected override void OnTarget(Mobile from, object target)
        {
            ...
            else if (target is BaseArmor || target is BaseClothing || target is BaseWeapon || target is BaseJewel)
            {
                ...
                if (item.LootType == LootType.Ensouled || item.LootType == LootType.Blessed || item.BlessedFor != null || (Mobile.InsuranceEnabled && item.Insured)) // Check if its already newbied (blessed)
