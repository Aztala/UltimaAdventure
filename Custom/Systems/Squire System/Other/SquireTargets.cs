...
namespace Server.Mobiles
{
	...
	public static void DoOnTarget( Mobile from, object o, Squire squire )
	{
		...
		if ( squire.InRange( item, 2 ) )
		{
			...
			else
			{
			    ...
			    squire.NextActionTime = DateTime.UtcNow.AddSeconds(5);
	...
	public static void DoOnTarget( Mobile from, object o, Squire squire )
	{
		...
		if ( o is Mobile )
		{
			...
			squire.m_ThrowDelay = DateTime.Now + TimeSpan.FromSeconds( 1 );
		    }
		    else
		    {
	...
	protected override void OnTarget( Mobile from, object provoketwo )
	{
		...
		{
		    m_Squire.NextSkillTime = DateTime.Now + TimeSpan.FromSeconds( 10.0 );
			...
			{
			    m_Squire.NextSkillTime = DateTime.Now + TimeSpan.FromSeconds( 5.0 );
			    if( m_Squire.m_SquireBeQuiet == false )//Added 1.8
			    {
	...
	{
	    m_Squire.RevealingAction();
	    m_Squire.NextSkillTime = DateTime.Now + TimeSpan.FromSeconds( 1.0 );
		...
		else if ( !targ.Player )
		{
		    ...
		    m_Squire.NextSkillTime = DateTime.Now + TimeSpan.FromSeconds( 12.0 );
		}
		else
		{
		    m_Instrument.PlayInstrumentBadly( m_Squire );
	...
	{
	    if ( m_SetSkillTime )
		from.NextSkillTime = DateTime.Now; //Updated NEXTVERSION for Compatibility
	}
