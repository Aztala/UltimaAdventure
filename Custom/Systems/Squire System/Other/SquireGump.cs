...
namespace Server.Gumps
{
	...
	public class SquireLoreGump : Gump
	{
		...
		private static string FormatSkill( BaseCreature c, SkillName name )
		{
            ...
            //			return String.Format( "<basefont color = #A3D7FF><div align='right'>{0:F1} <basefont color = #83d783>({1:F1})</basefont>/{2}</div></basefont>", skill.Value, skill.Base, skill.Cap ); 
            if (skill.Value == 20 && skill.Base < 20)
                return String.Format("<basefont color = #A3D7FF><div align='right'>{0:F1}/{1}</div></basefont>", skill.Base, skill.Cap);
            else
                return String.Format("<basefont color = #A3D7FF><div align='right'>{0:F1}/{1}</div></basefont>", skill.Value, skill.Cap);
		}
		...
		public SquireLoreGump( BaseCreature c, Mobile from, SquireLorePage page ) : base( 250, 50 )
		{
			...
			*/
			...
			else if ( page == SquireLorePage.Switches )
			{
                ...
                AddBackground(0, 0, 420, 594, 5054);
