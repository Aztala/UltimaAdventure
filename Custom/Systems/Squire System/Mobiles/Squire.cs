...
#region [Stats]Max
	...
	switch ( (command == null ? (int)SquireCommands.None : (int)command) )
	{
	    ...
	    case (int)SquireCommands.Lockpick: //New 1.8.5
		{
			...
			else
			{
			    ...
			    m_LockpickDelay = DateTime.Now + TimeSpan.FromSeconds(0.5);
			}
		    }
		    else
		    {
	    ...
	    case (int)SquireCommands.Confidence: // Rafa Bushido
		{
			...
			else
			{
				...
				if (DateTime.Now > m_SpellCastDelay)
				{
				    ...
				    m_SpellCastDelay = DateTime.Now + TimeSpan.FromSeconds(6.0);
				}
				else
				{
				    if (m_SquireBeQuiet == false)
	...
	if ( DateTime.Now > m_Delay )
	{
	    ...
	    if (Hits > 0 && AutoVampiricEmbrace && !TransformationSpellHelper.UnderTransformation(this, typeof(VampiricEmbraceSpell))
		    && NecroBook != null && DateTime.Now > m_SpellCastDelay && Skills.Necromancy.Value >= 99 && Mana >= 10)
	    {
		new Server.Spells.Necromancy.VampiricEmbraceSpell(this, null).Cast();
		m_SpellCastDelay = DateTime.Now + TimeSpan.FromSeconds(6.0);
	    }

	    if (Hits > 0 && m_AutoLoot && DateTime.Now > m_AutoLootDelay)
	    {
		m_AutoLootDelay = DateTime.Now + TimeSpan.FromSeconds(0.5);
		GrabItems(true, true, true);
	    }

	    if (m_AutoUseHealthPotion == true && healthPotion != null && DateTime.Now > m_HealthDelay && Hits <= (HitsMax / 2) && this.Poisoned == false
		...
		if( m_SquireBeQuiet == false )
		{
		    ...
		    && DateTime.Now > m_SpellCastDelay && Hits <= ((HitsMax / 3) * 2) && this.Poisoned == false && !MortalStrike.IsWounded(this)) // Added 1.9.6
	    {
		new Server.Spells.Chivalry.CloseWoundsSpell( this, null ).Cast();
		...
		if( m_SquireBeQuiet == false )
		{
		    ...
		    && DateTime.Now > m_SpellCastDelay && Hits >= ((HitsMax / 3) * 2) && this.Poisoned == true) // Added 1.9.6
	    {
		new Server.Spells.Chivalry.CleanseByFireSpell( this, null ).Cast();
		...
		if ( InRange( ControlMaster, Bandage.Range ) )
		{
			...
			if ( m.Alive == true && m.Poisoned )
			{
			    ...
			    if (m_AutoCastCleanseByFireMaster == true && ChivalryBook != null && !IsDeadPet && Mana >= 10 && TithingPoints >= 10
				    && DateTime.Now > m_SpellCastDelay && Hits >= ((HitsMax / 3) * 2) && m.Poisoned == true) // Added 1.9.7
			    {
				new Server.Spells.Chivalry.CleanseByFireSpell( this, null ).Cast();
				...
				&& TithingPoints >= 10 && DateTime.Now > m_SpellCastDelay && m.Hits <= ((m.HitsMax / 3) * 2) && m.Poisoned == false
			...
			else if ( m.Alive == false )
			{
			    ...
			    if ( m_AutoUseSpiritSpeak == true && DateTime.Now > m_SpiritDelay ) // Added 1.9.2
			    {
				SquireSpeaksWithSpirits();
			    }
	...
	// EM 2018.12.18 if( DateTime.Now > m_HungerDecay )
	if (DateTime.Now > m_HungerDecay && 1 != 1) // Block this routine
	{
	    if( this != null && Hunger >= 1 )
	    {
		Hunger -= 1;
		m_HungerDecay = DateTime.Now + TimeSpan.FromMinutes(50);
	    }
	    else if( DateTime.Now > m_AnnoyanceDelay )
	    {
		Emote( "Stomach Growls Audibly" );
		m_AnnoyanceDelay = DateTime.Now + TimeSpan.FromSeconds(300);
		if( Female )
		{
		    PlaySound( 0x313 );
		}
	...
	foreach ( Item item in items )
	{
		...
		if (!item.Stackable || !m_ConvenientStackable || !Backpack.TryDropItem(this, item, false))
		{
		    ...
		    NextActionTime = DateTime.UtcNow.AddSeconds(5);
			...
			if ( bc.Alive == true && bc.Poisoned )
			{
				...
				if ( m_AutoHealOther == true )
				{
				    if (m_AutoCastCleanseByFireAlly == true && ChivalryBook != null && !IsDeadPet && Mana >= 10 && TithingPoints >= 10
					    && DateTime.Now > m_SpellCastDelay && Hits >= ((HitsMax / 3) * 2) && bc.Poisoned == true) // Added 1.9.7
				    {
					new Server.Spells.Chivalry.CleanseByFireSpell( this, null ).Cast();
			...
			else if ( bc.Alive == true && bc.Hits < bc.HitsMax - 10 )
			{
				...
				if ( m_AutoHealOther == true )
				{
				    if (m_AutoCastCloseWoundsAlly == true && ChivalryBook != null && !IsDeadPet && Mana >= 10 && TithingPoints >= 10
					    && DateTime.Now > m_SpellCastDelay && bc.Hits <= ((bc.HitsMax / 3) * 2) && bc.Poisoned == false && !MortalStrike.IsWounded(bc)) // Added 1.9.7
				    {
					new Server.Spells.Chivalry.CloseWoundsSpell( this, null ).Cast();
