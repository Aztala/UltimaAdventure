			...
			if ( okay && house.IsOwner( from ) && !house.HasRentedVendors )
			{
				...
				if ( e != null )
				{
					...
					else if ( cost < 0 )
					{
						...
						if ( (DateTime.Now - house.BuiltOn) > TimeSpan.FromHours( 24 ) )
						{
							double chance = Utility.RandomDouble();

							if (chance <= 0.50)
							{
								from.SendMessage( "You list the house for some time, and someone buys it, but at a loss." );
								cost = (int)((double)cost * 0.90);
							}
							else if (chance <= 0.85)
							{
								from.SendMessage( "You list the house and someone buys it, but at a small loss." );
								cost = (int)((double)cost * 0.95);
							}
							else if (chance <= 0.97)
							{
								from.SendMessage( "An eager buyer for the home is found!  You sold at a small profit!" );
								cost = (int)((double)cost * 1.05);
							}
							else if (chance <= 1)
							{
								from.SendMessage( "There was a bidding war for the home and you sell it for a tidy profit!" );
								cost = (int)((double)cost * 1.1);
							}
						}
