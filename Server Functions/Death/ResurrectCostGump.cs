...
namespace Server.Gumps
{
	public class ResurrectCostGump : Gump
	{
		...
		public ResurrectCostGump( Mobile owner, int healer ) : base( 150, 50 )
		{
			...
			if ( ((PlayerMobile)owner).HardCore )
			{
			...
			if (owner.Karma >= 0)
				penalty = (((double)AetherGlobe.DoomCurse / 100000.0) * ( ((double)karma / 15000))) /1.5 ; // range of 0.0 to 0.66
			else
				penalty =  (( (100000-(double)AetherGlobe.DoomCurse) / 100000.0) * ( ((double)karma / 15000))) /1.5;
			...
			string c2 = "5";
			if ( !((PlayerMobile)owner).NormalMode)
				c2  = "10";
