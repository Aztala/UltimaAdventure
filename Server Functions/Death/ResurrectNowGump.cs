...
{ 
	...
	{ 
		...
		private static void ResurrectNow( object state )
		{
			...
			if ( ((PlayerMobile)m).LastAutoRes == null || DateTime.Now >= ( ((PlayerMobile)m).LastAutoRes + TimeSpan.FromMinutes( 60.0 ) ) )
			{
				((PlayerMobile)m).LastAutoRes = DateTime.Now;
	...
	public class ResurrectNowGump : Gump
	{
		public ResurrectNowGump( Mobile from ): base( 50, 20 )
		{
			...
			double penalty = 0;
				...
				if (from.Karma >= 0)
					penalty = ( (100 - ( ((double)AetherGlobe.DoomCurse / 100000.0) * ( ((double)from.Karma / 15000) ) )  ) / 100 ) ;
				else 
					penalty = ( (100 - (((double)(100000-AetherGlobe.DoomCurse) / 100000.0) * ( ((double)Math.Abs(from.Karma) / 15000) ) ) ) / 100 ) ;
