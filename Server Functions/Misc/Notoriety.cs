	...
	public class NotorietyHandlers
	{
		...
		public static int CorpseNotoriety( Mobile source, Corpse target )
		{
			...
			if( cretOwner != null )
			{
				...
				if( target.Kills >= 2 || (body.IsMonster && IsSummoned( target.Owner as BaseCreature )) || (target.Owner is BaseCreature && (((BaseCreature)target.Owner).AlwaysMurderer || ((BaseCreature)target.Owner).IsAnimatedDead)) )
		...
		public static int MobileNotoriety( Mobile source, Mobile target )
		{
			...
			if ( target.Kills >= 2 || ( target.Body.IsMonster && IsSummoned( target as BaseCreature ) && !( target is BaseFamiliar ) && !( target is Golem ) ) || ( target is BaseCreature && ( ( (BaseCreature)target ).AlwaysMurderer || ( (BaseCreature)target ).IsAnimatedDead ) ) )
				return Notoriety.Murderer;
