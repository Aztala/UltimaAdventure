...
namespace Server.Items
{
	public class TaskManagerDaily : Item
	{
		...
		public static void RunThis()
		{
			...
			if ( DateTime.Now > ( lastrun + TimeSpan.FromHours( 23.0 )) ) 			
			{
...
// Checking on investments	
				...
				ArrayList stones = new ArrayList(); // start finding investments
				...
				for ( int i = 0; i < players.Count; ++i )
				{
						...
						if (m.BalanceEffect < 0 && m.NormalMode && m.BalanceStatus != 0 && m.Kills == 0 ) //contributed to good
						{
						...
						else if (m.BalanceEffect > 0 && m.NormalMode && m.BalanceStatus != 0 ) //contributed to evil
						{
